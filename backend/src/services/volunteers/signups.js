...
function countEnrolled(eventId) {
...
export function registerForEvent(corporateTenantId, eventId, userId) {
  ...
  const existing = db.select().from(volunteerSignups)
    .where(and(eq(volunteerSignups.eventId, eventId), eq(volunteerSignups.userId, userId))).get()
  ...
  if (existing?.status === 'cancelled') {
    db.update(volunteerSignups).set({ status: 'registered', registeredAt: now })
      .where(eq(volunteerSignups.id, existing.id)).run()
    shapeEvent(row)
...
export function cancelRegistration(corporateTenantId, eventId, userId) {
  ...
  db.update(volunteerSignups).set({ status: 'cancelled' })
    .where(eq(volunteerSignups.id, signup.volunteer_signups.id)).run()

  const event = db.select().from(volunteerEvents).where(eq(volunteerEvents.id, eventId)).get()
