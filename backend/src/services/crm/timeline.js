...
export async function getProjectTimeline(projectId) {
  ...
  const wfInstances = await db
    ...
    .where(or(and(eq(workflowInstances.entityType, 'project'), eq(workflowInstances.entityId, projectId))))
    .all()

  const milestoneIds = await db
    .select()
  ...
  for (const wf of wfInstances) {
    ...
    for (const ev of events) {
      items.push({
        ...
        summary: `${wf.entityType} workflow ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ ${ev.toStatus}`,
  ...
  const thread = await db.select().from(messageThreads).where(eq(messageThreads.projectId, projectId)).get()
  if (thread) {
    const msgs = await db
      .select({
        id: messages.id,
  ...
  const tasks = await db.select().from(projectTasks).where(eq(projectTasks.projectId, projectId)).all()
  for (const t of tasks) {
    items.push({
      id: t.id,
      type: 'task',
      action: `task.${t.status}`,
      summary: `Task "${t.title}" ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â ${t.status}`,
