...
import { listSavedNgos, saveNgo, unsaveNgo, createNgoInquiry } from '../../services/discovery/index.js'
import {
  getVolunteerSummary,
  listEvents,
  getEvent,
...
import { ragCopilotChat, ragRecommendNgos, reindexAllVectors, isNgoDiscoveryQuery } from '../../services/ai/rag.js'
import {
  addKpi,
  addBeneficiaryLog,
  addGeoUpdate,
...
import { kpiInputSchema, beneficiaryLogSchema, geoUpdateSchema, attachUpdateFilesSchema } from '../../schemas/impact.js'
...
import { createThreadSchema, postMessageSchema, createTaskSchema, updateTaskSchema } from '../../schemas/crm.js'
...
router.get(
  '/discovery/ngos',
  requirePermission(PERMISSIONS.DISCOVERY_READ),
...
router.post(
  '/projects/:id/updates/:uid/files',
  requirePermission(PERMISSIONS.PROJECTS_WRITE),
  validate(attachUpdateFilesSchema),
  (req, res, next) => {
    try {
...
router.post(
  '/projects/:id/kpis',
  requirePermission(PERMISSIONS.IMPACT_WRITE),
  validate(kpiInputSchema),
  (req, res, next) => {
    try {
...
router.patch(
  '/projects/:id/spent',
  requirePermission(PERMISSIONS.FUNDS_READ),
  validate(updateSpentSchema),
  (req, res, next) => {
    try {
...
router.patch(
  ...
  validate(updateCsrProfileSchema),
  (req, res, next) => {
    try {
...
router.get(
  ...
  validate(esgUnifiedQuerySchema, 'query'),
  (req, res, next) => {
    try {
...
router.post(
  '/funds/intelligence',
  requirePermission(PERMISSIONS.FUNDS_READ),
  validate(allocationIntelligenceSchema),
  async (req, res, next) => {
    try {
...
router.post(
  ...
  validate(createVolunteerEventSchema),
  async (req, res, next) => {
    try {
      const event = createEvent(req.user.tenantId, req.user.id, req.validated)
      await logMutation({
        req,
        action: 'volunteer.event.create',
        entityType: 'volunteer_event',
        entityId: event.id,
        after: event,
      })
      ok(res, event, 201)
...
router.post('/volunteers/check-in', validate(volunteerCheckInSchema), async (req, res, next) => {
  try {
    ...
    await logMutation({
      req,
      action: 'volunteer.checkin',
      entityType: 'volunteer_event',
      entityId: result.eventTitle,
      after: result,
    })
    ok(res, result)
  } catch (err) {
...
router.post(
  '/volunteers/events/:id/attendance/manual',
  requirePermission(PERMISSIONS.VOLUNTEERS_MANAGE),
  ...
  async (req, res, next) => {
    try {
      ...
      await logMutation({
        req,
        action: 'volunteer.attendance.manual',
        entityType: 'volunteer_event',
...
router.post(
  '/audit/export',
  requirePermission(PERMISSIONS.ACTIVITY_EXPORT),
...
router.post(
  ...
  requirePermission(PERMISSIONS.COMMUNICATIONS_READ),
...
router.post(
  '/copilot/chat',
  requirePermission(PERMISSIONS.COPILOT_USE),
