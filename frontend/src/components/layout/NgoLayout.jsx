...
export function NgoLayout() {
  ...
  async function handleLogout() {
    await logout()
    navigate(ROUTES.ngoLogin)
  }

  function isActive(href) {
    if (href === NGO_ROUTES.home) return location.pathname === href
  ...
  const sidebar = (
    <div className="flex flex-col h-full">
      <div className="flex items-center gap-2 px-4 h-16 border-b border-emerald-200 bg-emerald-50">
        <Leaf className="h-6 w-6 text-emerald-600 shrink-0" />
        <span className="font-bold text-slate-900 truncate">NGO Portal</span>
      </div>
      <nav className="flex-1 overflow-y-auto py-4 px-3 space-y-6" aria-label="NGO navigation">
        {visibleSections.map((section) => (
          <div key={section.label}>
            <p className="px-3 mb-2 text-xs font-semibold uppercase tracking-wider text-slate-400">{section.label}</p>
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <aside className="hidden lg:fixed lg:inset-y-0 lg:flex lg:w-64 lg:flex-col border-r border-slate-200 bg-white z-30">{sidebar}</aside>
      <aside className={clsx('fixed inset-y-0 left-0 z-50 w-64 bg-white border-r border-slate-200 transform transition-transform lg:hidden', sidebarOpen ? 'translate-x-0' : '-translate-x-full')}>{sidebar}</aside>
